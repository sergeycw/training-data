...
2a. **ОБЯЗАТЕЛЬНО прочитай `recent_activities[].chat_notes`** — атлет комментирует каждую
   тренировку, комментарии синкаются автоматически (cutoff 7 дней). Смотреть **серию**,
   а не последнюю запись. Симптомы недомогания в записях **перекрывают** числовой
   `readiness_decision` = go (Section 11 §Multi-Metric Conflict Resolution: при расхождении
   HRV/RHR с заявленным состоянием приоритет у заявленного; «хороший» TSB не перекрывает
   болезнь и усталость). Расхождение называть вслух
...
**И `recent_activities[].chat_notes` — при любом типе запроса, до выводов.** Комментарии
атлета к тренировкам объясняют аномалии, которые иначе читаются как форма (RPE 7 из-за
холода и дождя, длинные паузы между повторами «гора кончилась» — контекст исполнения,
а не fade), и первыми показывают недомогание, которого числовые сигналы ещё не видят.
...
1. **Readiness**: `readiness_decision` как baseline → go/modify/skip + какие сигналы сработали
   - **Поправка на низкий ACWR:** если ACWR <0.8 просто потому что был перерыв/отпуск — это детренированность, а не риск. Amber по этому поводу игнорировать, вслух назвать причину. (Апстрим v11.27 уже убрал low-side ACWR из сигналов; локальный sync 3.84 ещё выдаёт.)
   - **Поправка на дрейф базы HRV:** `baseline_7d` уезжает за атлетом. После перерыва сверять HRV ещё и с сезонной базой из DOSSIER §4, иначе устойчивый спад читается как amber вместо красного.
2. **Load trajectory**: CTL slope, stress tolerance (3–6). ACWR и monotony — **только описательно**, не как гейт (см. `context/METHODS_RESEARCH_2026-08.md` §2.3, §2.4). Ramp rate — ориентир, не потолок.
3. **Phase**: `phase_detection.phase` + confidence + `phase_duration_weeks` → нужен ли deload?
4. **Capability**: durability/EF/TID drift trends, `sustainability_profile` (что реально держит на целевой длительности), `power_curve_delta`
5. **Ретро текущей недели**: план vs факт, RPE trending, какие типы работы выполнены
6. **Прогрессия**: что прогрессировали на прошлой неделе? Один вектор — по умолчанию; на re-entry после перерыва ≥7 дней правило снимается
7. **Разнообразие**: последние 6 quality-сессий — какие `session_template` id? Если ≥4 одного архетипа → брать формат из `context/WORKOUT_LIBRARY.md` Часть B
...
2. **TSS-калибровка (КРИТИЧНО — повторяющийся косяк, см. `feedback_tss_calibration`)**: рабочая (build/specificity) неделя атлета = **810–928 TSS** (медиана факта май–июнь). Peak до ~920–930. Deload ~600–625. **691–750 TSS = это уровень DELOAD, НЕ рабочая неделя — не выдавать за норму.** Перед генерацией свериться с `history.json weekly_180d` (последние 4–6 недель) и поставить план ≈ факт, не ниже. Если получается <800 без явного deload-триггера — пересчитать.
   - **НЕ разгружать «заранее перед усталостью», если deload уже стоит на W+1.** Двойная разгрузка = выброшенная неделя. W+1 = deload (напр. командировка/Стамбул) → текущую держать ПОЛНОЙ (~880–900), потом разгрузка = суперкомпенсация. Объём добирать аэробными хвостами на Z2/long-дни, квалити не трогать.
3. **Типы quality sessions — ОБЯЗАТЕЛЬНО через библиотеку.** Выбирать из `context/WORKOUT_LIBRARY.md`, писать `session_template: <id>` в план. Не изобретать структуры вне библиотеки (SECTION_11 §11B.8).
   - **Правило разнообразия:** один и тот же `id` не чаще раза в 3 недели, пока не исчерпаны альтернативы того же `domain`.
   - **Проверка перед генерацией:** посмотреть последние 6 назначенных quality-сессий. Если ≥4 из них — «N повторов фиксированной длины на фиксированном %FTP», взять формат из Части B библиотеки (B1–B9).
   - Не повторять один тип адаптации два раза подряд.
4. **Прогрессия**: один вектор — intensity (+2-5% W) ИЛИ duration (+1 rep / +длительность) ИЛИ environmental. Два вектора — только при RI ≥ 0.8 + HRV within 10%.
   - **Исключение:** на re-entry после перерыва ≥7 дней правило одного вектора приостанавливается — объём и специфическая интенсивность восстанавливаются параллельно, гейт по readiness каждой сессии. (Первичной поддержки правила одного вектора в литературе не найдено — см. METHODS_RESEARCH §2.6.)
5. **Распределение**: Hard:Easy чередование, long ride обычно Сб
6. **Валидация плана** (обновлено 02.08.2026 по ресерчу, см. `context/METHODS_RESEARCH_2026-08.md`):
   - Volume ±10% от baseline
   - **TSS-коридор — ОРИЕНТИР, не гейт.** Гейт по измеряемому выходу (может ли держать ватты при ожидаемых ЧСС/RPE) + утренний readiness. CTL/TSB — описательная бухгалтерия, не решающее правило.
   - **Контроль всплеска отдельной сессии:** длинная не длиннее чем на 10–15% от самой длинной за 30 дней. Это единственное правило прогрессии нагрузки с когортной поддержкой.
   - **Нет back-to-back hard days** — по умолчанию. **Исключение:** намеренный концентрированный блок ≤7 дней, обязательно с 5–6 днями восстановления после. Условие «TSB>0» здесь не применять — блок по определению загоняет TSB в минус.
   - **Grey zone / Z3 НЕ минимизировать перед событием пороговой длительности.** В специфической фазе MIT (2.5–4.0 ммоль, 83–87% HRmax → ЧСС 154–162) — основной стимул. Ограничивать ценой восстановления, не долей.

   **УДАЛЕНО из валидации (не подтверждено или дискредитировано):**
   - ~~Поляризация ≥0.80~~ — метрика названа неверно (настоящий Treff PI имеет порог >2.00, значения 0–2.00 непорядковые); POL vs PYR не даёт разницы ни в VO2max (SMD −0.06), ни в TT (SMD −0.05). >90% профи WorldTour пирамидальны.
   - ~~ACWR ≤1.3~~ — метрика дискредитирована (случайный знаменатель воспроизводит эффект; c-статистика 0.574 ≈ 0.5). При ACWR <0.8 после перерыва правило подавляет нужный объём.
   - ~~Монотонность <2.5~~ — числа нет в первоисточнике; штрафует блоковую периодизацию, которая и есть лучший свежий протокол.
...
3. Блок(и) по активностям:
   ...
   - Температура (`avg_temp`) — при >28°C применять допуск HR +5–8 bpm из DOSSIER §7 (терморегуляционный дрифт, не усталость)
...
- Диапазон `- 60m 80-86% LTHR` парсится и даёт duration/load. Одиночное `- 60m 76% LTHR` тоже
- НЕ использовать абсолютные bpm (`128-143bpm`) — Intervals.icu их не парсит, тренировка создаётся без duration/load

**Два гейта в одном шаге (power + HR) — работает.** Проверено эмпирически 12.08.2026.
Порядок обязателен: **сначала мощность, потом пульс, потом каденс.**

```
- 10m 94% 96-101% LTHR 85-90rpm
```
→ `{"power": {"units":"%ftp","value":94}, "hr": {"units":"%lthr","start":96,"end":101}, "cadence": {...}}`

| Форма | Результат |
|---|---|
| `- 10m 94% 96-101% LTHR` | ✅ power + hr `%lthr` (точный диапазон) |
| `- 10m 94% Z4 HR` | ✅ power + hr `hr_zone` |
| `- 10m 94% 96-101% LTHR 85-90rpm` | ✅ power + hr + cadence |
| `- 10m 94% 154-162bpm` | ❌ bpm молча выброшен, остаётся только power |
| `- 10m 154-162bpm 94%` | ❌ то же — порядок не спасает |

**Абсолютные bpm не парсятся ни в каком положении.** Переводить в `% LTHR`
(LTHR = 160 → 154 bpm = 96 %, 140 bpm = 87 %, 125 bpm = 78 %).

**Когда ставить два гейта:** любая сессия на байке с PM, где ЧСС несёт отдельный смысл —
MIT (ЧСС первична, ватты проверочные), Z2 (ватты первичны, ЧСС cross-check на жару
и недовосстановление). Один гейт — только когда второй физически недоступен
(gravel без PM → чистый пульс). **В `description` явно писать, какой из двух первичен
при расхождении** — сам синтаксис этого не выражает.
