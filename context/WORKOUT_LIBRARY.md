# Workout Library — Sergey

**Источник:** Section 11 Workout Reference Library v0.6 (upstream, MIT) + athlete-specific расширение.
**Установлена:** 2026-08-02. До этой даты библиотеки в репозитории НЕ БЫЛО — при том что
SECTION_11 §11B.8 требует выбирать сессии из неё и запрещает изобретать структуры вне её.
Следствие: 21 из 22 quality-сессий за 5 месяцев были одним архетипом
(«N повторов фиксированной длины на фиксированном %FTP»).

## Ватты под текущий FTP (293 outdoor / 285 indoor)

| Зона | %FTP | Outdoor (293W) | Indoor (285W) |
|------|------|----------------|---------------|
| Z1 Recovery | 0–55% | ≤161 W | ≤157 W |
| Z2 Endurance | 56–75% | 164–220 W | 160–214 W |
| Z3 Tempo | 76–90% | 223–264 W | 217–256 W |
| Z4 Threshold | 91–105% | 267–308 W | 259–299 W |
| Z5 VO2max | 106–120% | 311–352 W | 302–342 W |
| Z6 Anaerobic | 121–150% | 355–440 W | 345–428 W |
| SS Sweetspot | 84–97% | 246–284 W | 239–276 W |

Часто используемые точки (outdoor / indoor):
65% = 190/185 · 75% = 220/214 · 83% = 243/237 · 88% = 258/251 · 94% = 275/268
95% = 278/271 · 98% = 287/279 · 100% = 293/285 · 105% = 308/299
110% = 322/314 · 113% = 331/322 · 118% = 346/336 · 125% = 366/356

## Правило разнообразия (athlete-specific)

Не назначать один и тот же `id` шаблона чаще, чем раз в 3 недели, пока не исчерпаны
альтернативы того же `domain`. При выборе сессии писать `session_template: <id>`
в аудит-метаданные плана (SECTION_11 §11B.6/§11B.8) — это делает однотипность
машинно-видимой вместо того, чтобы копиться незаметно.

Использовано за 5 месяцев до установки библиотеки: фактически только TH-1, SS-1, SS-5,
VO2-1 и один раз SS-3. Не использовано ни разу: AE-5, AE-6, AE-7, SS-2, SS-4, TH-2,
VO2-2, VO2-3, VO2-4, VO2-5, AN-1, AN-2, AN-3, MIX-1, MIX-2, SE-1, SE-2.
...
# ЧАСТЬ B — Athlete-specific расширение (ресерч 02.08.2026, B10 добавлен позже в тот же день)

Форматы, отсутствующие в апстрим-библиотеке. Все ватты под FTP 293 (outdoor).
TSS/IF — расчётные по Coggan (IF=NP/FTP, TSS=ч×IF²×100), оценка, не замер.
Метка `[СЕЙЧАС]` = применимо в окне до попытки 17–23.08. `[СЛЕД. ЦИКЛ]` = после.

## Почему эти форматы, а не ещё повторы порога

Sitko et al. 2022: медиана TTE на FTP — 35 мин любители / 42 тренированные /
47 хорошо тренированные / **51 мин профессионалы**. Его 50.5 мин @292 Вт (~100% FTP)
= профессиональный уровень TTE. **Полгода пороговых повторов развивали качество,
которым он уже владеет на элитном уровне.** Потолок держит не TTE, а аэробная мощность
и эффективность.

---

## B1. Фракционированный суб-порог (Norwegian short-rep) `[СЕЙЧАС]`

- **Структура:** WU 20 мин @161 Вт · **25×1 мин @293–299 Вт / 30 с @176 Вт** · CD 10 мин @147 Вт
- **~67 мин · ~63 TSS · IF ~0.75 · ЧСС в работе 158–164**
- **Смысл:** короткие паузы держат абсолютную мощность на пороге при меньшем накоплении лактата → больше рекрутирования ДЕ при низкой метаболической цене.
- **Доказательность:** механизм — практика (Casado et al. IJERPH 2023;20:3782). Косвенно: Bellinger 2026 — короткие повторы дают больше периферической деоксигенации (+3.42±2.23%, p=0.006; время >60% пика деокси +83.5±66.4 с, p=0.02) при равном RPE и лактате.
- **Ограничение:** нужна ровная дорога или станок. На подъёме 30-секундные паузы ломают темп.

## B2. Прогрессивный суб-порог (Bakken progressive) `[СЕЙЧАС]`

- **Структура:** WU 20 мин · **5×8 мин: 258 → 264 → 270 → 275 → 281 Вт, R=2 мин @150 Вт** · CD 10 мин
- **~72 мин · ~63 TSS · IF ~0.72 · ЧСС 150→162, RPE 14→16**
- **Смысл:** лактат растёт ~2.0 → ~3.0 ммоль в течение сессии. Разница первый/последний интервал 10–25 Вт (Tønnessen et al. Sports Med 2024;54:2935–53, 12 тренеров, >370 медалей).
- **Ограничение:** если первый повтор уже на 95% FTP — прогрессия невозможна, выродится в обычный порог.

## B3. Убывающая мощность / fast start (DEC) `[СЕЙЧАС]`

- **Структура:** WU 20 мин · **5×8 мин (1.5 мин @316 Вт → 6.5 мин @275 Вт), R=3 мин @147 Вт** · CD 10 мин
- **~82 мин · ~81 TSS · IF ~0.79**
- **Результат:** Rønnestad 2020 — DEC дал пик VO2 **98.2±2.1% vs 95.4±3.1%** VO2peak и средний **87.6% vs 86.1%**, при **более низком RPE (16.1 vs 16.5)**, все p<0.05, при одинаковой средней мощности.
- **Двойная польза:** прямая репетиция стартового отрезка подъёма.
- **Важно:** это тренировочный приём. В самой попытке fast start расходует W′ — не гоночная тактика.

## B4. Вариативные рабочие интервалы (Bossi) `[СЕЙЧАС, 1 раз]`

- **Структура:** WU 20 мин · **4×10 мин, внутри — 4 блока по (40 с @340 Вт + 60 с @270 Вт) + 2.5 мин @270 Вт, R=4 мин @150 Вт** · CD 10 мин
- **~82 мин · ~78 TSS · IF ~0.77**
- **Результат:** Bossi et al. 2020 — время >90% VO2max **410±207 с vs 286±162 с (p=0.02)** при одинаковой длительности и средней мощности; метрики нагрузки по ЧСС и RPE **не различались** (p≥0.1).
- **Это буквально бесплатно:** та же сессия, тот же TSS, больше времени на высоком %VO2max.
- **Ограничение:** обманчиво низкий session RPE — легко перебрать частоту.

## B5. Порог на низком каденсе `[СЕЙЧАС, аккуратно]`

- **Структура:** WU 20 мин @161 Вт (нормальный каденс) · **4×10 мин @270 Вт при 55–60 rpm, R=4 мин @147 Вт** · CD 10 мин
- **~82 мин · ~76 TSS · IF ~0.76**
- **Результат:** Hebisz & Hebisz 2024 (PLoS One 19(11):e0311833) — прирост Pmax значим только в низкокаденсной группе; VT2 изменился только в ней (χ²=11, p=0.001).
- **КРИТИЧЕСКАЯ ОГОВОРКА:** в исследовании низкий каденс применялся к SIT/HIIT, не к порогу. Kristoffersen: 12 недель 5×6 мин @40 rpm у тренированных ветеранов дали **ноль**, тогда как свободный каденс улучшил. Устойчивая езда на низком каденсе не превышает **<54% максимальной динамической силы** — ниже порога силовой адаптации. **Низкий каденс имеет смысл только на максимальных/околомаксимальных усилиях, не на темпе.**
- **Ограничение:** ≤1 сессия/нед, колени, не совмещать с силовой на ноги.

## B6. Порог в конце длинной работы (durability) `[СЕЙЧАС — приоритет повышен 02.08.2026]`

> **Смена статуса.** При новой иерархии целей (одиночный заезд 12–20 ч — приоритет №1)
> durability перестала быть «следующим циклом» и стала основным качеством.
> Применять с W35 (после того как длинная дорастёт до 10 ч), 1 раз в 2–3 недели,
> и **никогда на самой длинной ступени** — только на средней.


- **Структура:** 2.5 ч @190 Вт (ЧСС <140) · **3×10 мин @281 Вт, R=5 мин @150 Вт** · CD 15 мин
- **~3 ч 40 мин · ~162 TSS · IF ~0.66 · углеводы 90–120 г/ч**
- **Смысл:** Spragg 2023 — производительность в утомлённом состоянии частично отдельное тренируемое качество.
- **ЧЕСТНАЯ ОГОВОРКА:** RCT, сравнивающего интервалы в утомлённом состоянии с интервалами на свежести, **не существует**. Более того, Spragg 2024: у профи 12-минутная мощность **полностью сохранилась** после 2000 кДж (390→386 Вт, ns). Деградируют 1-с (−9%), 15-с (−10%) и W′ — длительности, которые ему не нужны.
- **Ценность для него в первую очередь как репетиция и калибровка пейсинга**, а не адаптация.

## B7. Специфическая race-simulation `[СЕЙЧАС, 1–2 раза]`

- **Структура:** WU 20 мин @161 Вт · **2×20 мин @287–293 Вт на реальном подъёме, смена сидя/стоя каждые 5 мин, R=8 мин @147 Вт** · CD 10 мин
- **~78 мин · ~83 TSS · IF ~0.80 · ЧСС 160–166**
- **Обоснование сидя/стоя:** Carlsson et al. 2024 (Front Sports Act Living 6:1395415) — у 10 элитных велосипедистов повторные переходы сидя↔стоя дали **значимо более низкий пост-лактат** при отсутствии различий в VO2, метаболической мощности и валовой эффективности.
- **Ограничение:** высокая психологическая цена, ≤1 раз в 7–10 дней.

## B8. MIT-блок («серая зона» концентрированно) `[СЕЙЧАС — ядро плана]`

- **Структура блока:** 6 сессий за 7 дней, каждая 5–7×10–14 мин @ ~2.8 ммоль (≈85% HRmax → **ЧСС 154–162**, мощность ≈258–275 Вт), затем **6 дней активного восстановления**
- **Результат:** Mølmen et al. 2025 (MSSE, DOI 10.1249/MSS.0000000000003706) — n=30, VO2max 70.5±4.6, рандомизированный **кроссовер**, TRIMP уравнен. Мощность на 4 ммоль **+4.0±4.4% vs −1.3±3.7% (p<0.01)**; POVO2max **+2.5% vs −0.7% (p<0.01)**; VO2max **+2.0% vs 0.0% (p=0.05)**.
- **MIT > HIT для его цели:** Rønnestad et al. 2025 (Eur J Sport Sci, DOI 10.1002/ejsc.70067) — мощность на 4 ммоль **+4.5% vs +2.1% (p=0.03)** в пользу MIT.
- **Это ровно та «серая зона», которую его протокол запрещал.** Для события длительностью 48 мин на ~98% FTP — самый специфичный стимул из найденных.

## B9. Полный тест-прогон (dress rehearsal) `[СЕЙЧАС — обязательно]`

- **Структура:** реальный подъезд к подъёму на реальной мощности + **полные 48 мин на сегменте** + возврат. Реальное питание, реальное время суток, реальная жара.
- **~3 ч · ~160 TSS · IF ~0.73**
- **Зачем:** индивидуальный разброс огромен. Jones 2024 — падение CP после 2 ч от **<1% до ~32%** между людьми. Valenzuela 2023 — распад 20-мин TT у 12 профи от **−8.5% до +1.1%** (некоторые быстрее на усталости), и **ни одна лабораторная или тренировочная переменная не предсказала, кто есть кто**.
- **Следствие:** его цифру нельзя оценить из теории, только измерить. Если распад +1% — пейсинг 292 Вт. Если −7% — 288 Вт фантазия, реальный таргет 272 Вт.

## B10. Самотемповое исполнение (self-paced) `[СЕЙЧАС — 1 раз в неделю, не чаще]`

- **Структура:** WU 20 мин @161 Вт · **6×5 мин «максимальное усилие, которое дотянешь до конца шестого повтора», R=2.5 мин @150 Вт** · CD 10 мин @147 Вт
- **~72 мин · ~78 TSS · IF ~0.78** (оценка; фактический IF будет выше планового — это ожидаемо)
- **Правило исполнения — единственное, что важно:** **таргета в ваттах НЕТ.** Атлет едет по ощущению, задача — равномерно распределить максимум по всем шести повторам. Ватты пишутся, но во время сессии на них не смотреть; экран убрать или переключить на время.
- **Результат:** Appelhans, Rønnestad & Skovereng 2025 (IJSPP, **PMID 40328438**), n=12, VO2max 68±6.3, кроссовер. Время выше 90 % VO2max в формате 6×5 мин: **178±159 с при фиксированной мощности → 1016±312 с самотемпово. В 5.7 раза.** Все физиологические параметры и мощность выше в самотемповом режиме (все p<0.001, все ηp²>0.795).
- **Побочный вывод той же работы:** при фиксированной мощности короткие интервалы били длинные (p<0.01), **а при самотемповом исполнении разница между форматами исчезала (p=0.321)**. Спор «короткие vs длинные» — во многом артефакт способа задания интенсивности.
- **Почему это его формат:** все его квалити-сессии за шесть месяцев были на фиксированном таргете в ваттах. По этим данным он мог получать порядка одной пятой доступного стимула VO2 от каждой длинной сессии.
- **ОГОВОРКИ.** Острый замер, не тренировочная интервенция — 5.7× по времени >90 % VO2max не означает 5.7× адаптации, работа этого не заявляет. Фактическая мощность выше → **стоимость восстановления выше**: считать полноценной тяжёлой сессией, не «тем же самым, но лучше». Не более **1 раза в неделю**. Не ставить за 48 ч до квалити или репетиции.
- **Как оценивать постфактум:** смотреть на распределение мощности между повторами. Падение с 1-го по 6-й больше 8 % = стартовал слишком высоко, в следующий раз начинать сдержаннее. Рост от 1-го к 6-му = недотерпел, есть запас.
- **Не путать с B3 (fast start).** B3 задаёт форму мощности внутри повтора при фиксированных ваттах; B10 снимает фиксацию ватт вообще.

---

## Что НЕ делать (развенчано ресерчем)

| Метод | Вердикт |
|---|---|
| **Over-unders 60/60** | Urianstad 2024: 60 с @110% / 60 с @90% **не превзошёл постоянную мощность** (85.6% vs 85.0% VO2max, p≥0.36). Fennell & Hopker 2020: активное восстановление снижает мощность рабочих интервалов и **не улучшает клиренс лактата**. Нужны fast-start и внутриинтервальная вариация, а не оверы-андеры. |
| **«Sweet spot» как методика** | Ноль публикаций в спортивной науке под этим названием. `[UNVERIFIED]` |
| **Снижение vLamax поднимает FTP** | Ни одной интервенции не найдено. Quittmann 2026: предсказательная ценность для ≥1 мин «inconclusive». |
| **Сауна / горячая ванна для жаровой адаптации** | Мета: ratio of means **1.04 (95% ДИ 0.94–1.15, p=0.46)**. Добавление HWI к 3-недельному HIIT-блоку дало between-group **ES = 0.0**. Пассивный нагрев не работает — работает активный (езда в лишней одежде). |
| **Изометрия (wall sits) для велоспорта** | Пиковый момент +14.2%, но «не транслировалось в изменения результата или экономичности» (p>0.131). |
| **Нос-дыхание для результата** | Исключительно носовое дыхание значимо **снижает** пиковый VO2, VE и мощность. |
| **Гриндинг 50 rpm на темпе** | <54% максимальной динамической силы — ниже порога силовой адаптации. Не силовая работа и, по данным, вообще почти ничего. |
| **Мотопейсинг, AI-пейсинг** | Ноль опубликованных исследований. Отвергнуто как недоказанное. |
